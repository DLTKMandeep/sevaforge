"""
SevaForge Model Router — Multi-Provider LLM Gateway

Routes requests to Anthropic, OpenAI, Google, or Mock providers based on
model name.  Each provider is wrapped in an independent circuit breaker so
a single-provider outage triggers automatic failover to the configured
fallback model.
...
class GoogleAdapter:
    ...
    async def call(
        ...
        try:
            ...
            # Gemini expects a flat contents list; system messages become a
            # preamble in the first user turn.
            ...
            # Approximate token counts from character lengths (Gemini does not
            # always return usage metadata consistently).
            input_text = " ".join(
...
class ModelRouter:
    """
    Multi-provider LLM router with circuit breaker failover.
    ...
    The class satisfies the ``call(messages, model, max_tokens) -> str``
    interface expected by :class:`sevaforge.gateway.ai_gateway.AIGateway`.
    """
    ...
    def __init__(
        ...
        *,
        ...
        logger.info(
            "ModelRouter initialised — adapters=[%s] default=%s fallback=%s",
            available,
            self._default_model,
    ...
    async def _call_with_retry(
        ...
        # The breaker's `.state` property handles the OPEN -> HALF_OPEN
        # transition automatically based on elapsed time.
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<ModelRouter providers=[{providers}] "
