...
_VULN_PATTERNS = {
    ...
    "cwe-327": {
        ...
        "patterns": [
            ...
            r"DES\.",
            r"RC4\.",
            r"random\.random\s*\(",   # non-cryptographic PRNG
        ],
        "severity": "high",
    },
    "cwe-295": {
...
risks without evidence in the code."""
    ...
    async def execute(self, ctx: AgentExecutionContext) -> dict[str, Any]:
        ...
        ctx.input: code or configuration to analyze
        ctx.params:
          - scan_type: "full" | "secrets" | "vulnerabilities" | "compliance"
          - language: str
          - file_path: str
...
Focus on issues that require semantic understanding (logic flaws, auth bypasses, race conditions, etc.)."""
    ...
    def _parse_llm_security_findings(self, text: str) -> list[dict[str, Any]]:
        """Parse security findings from LLM output."""
        findings = []
