...
async def register_tool(request: Request) -> dict:
    ...
    capabilities = [
        ToolCapability(name=c.get("name", ""), description=c.get("description", ""), input_schema=c.get("input_schema", {}), output_schema=c.get("output_schema", {}))
        for c in body.get("capabilities", [])
    ]
    tool_def = ToolDefinition(name=body.get("name", ""), description=body.get("description", ""), version=body.get("version", "1.0.0"), capabilities=capabilities, tags=body.get("tags", []), category=body.get("category", "general"), author=body.get("author", ""))
...
@router.get("/")
async def list_tools(category: str | None = None, tag: str | None = None, active_only: bool = True) -> list[dict]:
    registry = get_registry()
...
async def search_tools(request: Request) -> list[dict]:
    ...
    registry = get_registry()
    results = registry.search_tools(query, top_k=body.get("top_k", 5))
    return [{"tool": tool.to_dict(), "similarity_score": round(score, 4)} for tool, score in results]


@router.post("/connectors/register")
async def register_connector(request: Request) -> dict:
    body = await request.json()
    config = ConnectorConfig(name=body.get("name", ""), base_url=body.get("base_url", ""), auth_scheme=AuthScheme(body.get("auth_scheme", "none")), auth_config=body.get("auth_config", {}), default_headers=body.get("default_headers", {}), timeout_seconds=body.get("timeout_seconds", 30), retry_max=body.get("retry_max", 3), retry_backoff_factor=body.get("retry_backoff_factor", 0.5), rate_limit_rpm=body.get("rate_limit_rpm", 60))
