...
class BaseAgent(abc.ABC):
    ...
    async def call_llm(
        ...
        Two modes:
        - **Explicit model** (default): Uses the agent's default model
          unless overridden by ``ctx.model_override`` or ``model``.
        - **Auto-route** (``auto_route=True``): Lets the multi-router
          intelligently select the best model based on task complexity,
          cost, and capability requirements.  Pass ``model=None`` to
          enable this mode, or set ``auto_route=True``.

        Returns:
            ProviderResponse with content, token counts, latency,
            routing_decision (when auto-routed), and cost_usd.
        """
        # Determine model: None means let the router decide
        if auto_route:
            target_model = None  # Router will classify and pick
        else:
        ...
        # Use call_advanced for full routing intelligence
        response = await self._get_model_router().call_advanced(
            messages=messages,
            model=target_model,
