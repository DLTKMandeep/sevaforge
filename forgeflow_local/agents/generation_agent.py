...
ARGOCD_APPLICATION = '''# ArgoCD Application Manifest
...
ARGOCD_PROJECT = '''# ArgoCD AppProject
# Generated by ForgeFlow
apiVersion: argoproj.io/v1alpha1
kind: AppProject
metadata:
...
spec:
  ...
  # Allow manifests from any Git repository
  sourceRepos:
    - '*'
  
  # Allow deployment to these namespaces
  destinations:
  ...
  roles:
    ...
    - name: developer
      description: Developer access (read + sync)
      policies:
...
KUSTOMIZE_BASE = '''# Kustomize Base Configuration
# Generated by ForgeFlow
...
KUSTOMIZE_OVERLAY = '''# Kustomize Overlay for {environment}
# Generated by ForgeFlow
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization
...
K8S_CONFIGMAP = '''# Kubernetes ConfigMap
# Generated by ForgeFlow
apiVersion: v1
kind: ConfigMap
metadata:
...
K8S_HPA = '''# Kubernetes Horizontal Pod Autoscaler
# Generated by ForgeFlow
...
DEPLOYMENT_PATCH_TEMPLATE = '''# Environment-specific deployment patch
# Generated by ForgeFlow
apiVersion: apps/v1
kind: Deployment
metadata:
...
class GenerationAgent(BaseAgent):
    ...
    def execute(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Generate ArgoCD manifests and Kustomize overlays
        argocd_files = self._generate_argocd(infra_path, app_name, primary_lang)
        artifacts.extend(argocd_files)
        generated_files.extend([f['file'] for f in argocd_files if f['status'] == 'generated'])
        ...
        if inventory_file.exists():
            try:
                with open(inventory_file) as f:
                    data = json.load(f)
                    # Handle both formats: {"inventory": [...]} or just [...]
    ...
    def _detect_primary_language(self, repo_path: Path, discover_results: Optional[Dict]) -> str:
        ...
        if discover_results and 'inventory' in discover_results:
            languages = {}
            inventory = discover_results['inventory']
...
'''
    ...
    def _generate_argocd(
        self,
        infra_path: Path,
        app_name: str,
        primary_lang: str,
        repo_url: str = "https://github.com/your-org/your-repo.git"
    ) -> List[Dict[str, Any]]:
        """Generate ArgoCD manifests and Kustomize overlays."""
        results = []
        ...
        argocd_path = infra_path / 'argocd'
        argocd_path.mkdir(parents=True, exist_ok=True)
        ...
        # Environment configs
        environments = {
            'dev': {'replicas': 1, 'log_level': 'debug', 'cpu_request': '100m', 'memory_request': '128Mi', 'cpu_limit': '500m', 'memory_limit': '512Mi'},
            'staging': {'replicas': 2, 'log_level': 'info', 'cpu_request': '200m', 'memory_request': '256Mi', 'cpu_limit': '1000m', 'memory_limit': '1Gi'},
            'prod': {'replicas': 3, 'log_level': 'warn', 'cpu_request': '500m', 'memory_request': '512Mi', 'cpu_limit': '2000m', 'memory_limit': '2Gi'},
        }
        
        # Generate ArgoCD AppProject
        appproject_path = argocd_path / 'appproject.yaml'
        if not appproject_path.exists():
            appproject_path.write_text(ARGOCD_PROJECT.format(app_name=app_name))
            results.append({
                'file': 'infrastructure/argocd/appproject.yaml',
                'type': 'argocd',
                'status': 'generated',
                'description': 'ArgoCD AppProject with RBAC'
            })
        else:
            results.append({'file': 'infrastructure/argocd/appproject.yaml', 'type': 'argocd', 'status': 'exists'})
        
        # Generate ArgoCD Application for each environment
        for env in environments.keys():
            app_file = argocd_path / f'application-{env}.yaml'
            if not app_file.exists():
                app_file.write_text(ARGOCD_APPLICATION.format(
                    app_name=f"{app_name}-{env}",
                    repo_url=repo_url,
                    environment=env
                ))
                results.append({
                    'file': f'infrastructure/argocd/application-{env}.yaml',
                    'type': 'argocd',
                    'status': 'generated',
                    'description': f'ArgoCD Application for {env}'
                })
            else:
                results.append({'file': f'infrastructure/argocd/application-{env}.yaml', 'type': 'argocd', 'status': 'exists'})
        
        # Generate Kustomize base
        base_files = {
            'kustomization.yaml': KUSTOMIZE_BASE.format(app_name=app_name),
            'deployment.yaml': K8S_DEPLOYMENT.format(
                app_name=app_name,
                image=f"ghcr.io/your-org/{app_name}",
                tag="latest",
                port=app_port
            ),
            'service.yaml': K8S_SERVICE.format(app_name=app_name, port=app_port),
            'configmap.yaml': K8S_CONFIGMAP.format(app_name=app_name),
            'hpa.yaml': K8S_HPA.format(app_name=app_name),
        }
        ...
        for filename, content in base_files.items():
            ...
            if not file_path.exists():
                file_path.write_text(content)
                results.append({
                    'file': f'infrastructure/k8s/base/{filename}',
                    'type': 'kubernetes',
                    'status': 'generated',
                    'description': f'K8s base {filename}'
                })
            else:
                results.append({'file': f'infrastructure/k8s/base/{filename}', 'type': 'kubernetes', 'status': 'exists'})
        
        # Generate Kustomize overlays for each environment
        for env, config in environments.items():
            overlay_path = k8s_path / 'overlays' / env
            overlay_path.mkdir(parents=True, exist_ok=True)
            
            # Kustomization.yaml
            kust_file = overlay_path / 'kustomization.yaml'
            if not kust_file.exists():
                kust_file.write_text(KUSTOMIZE_OVERLAY.format(
                    app_name=app_name,
                    environment=env,
                ...
                results.append({
                    'file': f'infrastructure/k8s/overlays/{env}/kustomization.yaml',
                    'type': 'kustomize',
                    'status': 'generated',
                    'description': f'Kustomize overlay for {env}'
                })
            
            # Deployment patch
            patch_file = overlay_path / 'deployment-patch.yaml'
            if not patch_file.exists():
                patch_file.write_text(DEPLOYMENT_PATCH_TEMPLATE.format(
                    app_name=app_name,
                ...
                results.append({
                    'file': f'infrastructure/k8s/overlays/{env}/deployment-patch.yaml',
                    'type': 'kustomize',
                    'status': 'generated',
                    'description': f'Resource limits for {env}'
                })
