...
async def record_audit(request: Request) -> dict:
    ...
    audit = get_audit()
    entry = audit.record(action=action, actor_id=actor_id, actor_type=actor_type, resource_type=body.get("resource_type", ""), resource_id=body.get("resource_id", ""), details=body.get("details", {}), outcome=body.get("outcome", "success"), tenant_id=body.get("tenant_id", "default"), ip_address=body.get("ip_address", ""), trace_id=body.get("trace_id", ""))
    return entry.to_dict()


@router.get("/audit/query")
async def query_audit(actor_id: str | None = None, action: str | None = None, resource_type: str | None = None, tenant_id: str | None = None, limit: int = 100) -> list[dict]:
    audit = get_audit()
    action_enum = None
    if action:
        try:
    ...
    entries = audit.query(actor_id=actor_id, action=action_enum, resource_type=resource_type, tenant_id=tenant_id, limit=limit)
...
async def verify_audit_entry(entry_id: str) -> dict:
    ...
    return {"entry_id": entry_id, "integrity_valid": is_valid, "checksum": entry.checksum}
